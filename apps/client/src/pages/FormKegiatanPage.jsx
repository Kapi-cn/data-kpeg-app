	...
	return (
		<div class=":uno: layout-light flex items-center gap-3 py-3 border-b border-dashed border-[var(--border)]">
			...
			<div class=":uno: space-y-1">
				<h2 class=":uno: text-[var(--title)] font-medium text-xs uppercase tracking-tight">
					{props.title}
				</h2>
				<p class=":uno: text-[11px] leading-3.5 text-[var(--text-muted)]">
					{props.description}
				</p>
			</div>
		</div>
...
function BalaiForm(props) {
	return (
		<div class=":uno: layout-light max-w-3xl mt-5 rounded-3xl p-6 bg-[var(--surface)] shadow-md space-y-6">
			<CardHeader
				order={1}
				title="Identitas Kegiatan Balai"
				description="Isi agenda resmi kegiatan balai"
			/>
			<div class=":uno: space-y-4">
				<div>
					<label
						for="nama-kegiatan-balai"
						class=":uno: block mb-1.5 text-xs text-[var(--title)]/80 font-semibold"
					>
						Nama Kegiatan
					</label>
					<input
						id="nama-kegiatan-balai"
						type="text"
						value={props.namaKegiatan}
						onInput={(event) =>
							props.setNamaKegiatan(event.currentTarget.value)
						}
						class=":uno: py-2.5 px-3 text-xs text-[var(--text)] w-full bg-slate-50/50 border border-slate-200 rounded-xl focus:(border-primary/80 outline-2 outline-primary/20)"
						placeholder="Contoh: Rapat Evaluasi X"
					/>
				</div>
				<div class=":uno: grid grid-cols-1 sm:grid-cols-2 gap-4">
					<div>
						...
						<input
							...
							type="date"
							value={props.tanggal}
							onInput={(event) => props.setTanggal(event.currentTarget.value)}
							class=":uno: py-2.5 px-3 text-xs text-[var(--text)] w-full bg-slate-50/50 border border-slate-200 rounded-xl"
						/>
					</div>
					<div>
						<label
						...
						<input
							...
							type="time"
							value={props.jam}
							onInput={(event) => props.setJam(event.currentTarget.value)}
							class=":uno: py-2.5 px-3 text-xs text-[var(--text)] w-full bg-slate-50/50 border border-slate-200 rounded-xl"
						/>
					</div>
				</div>
				<div>
					...
					<div class=":uno: relative">
						<FiMapPin
							class=":uno: absolute left-3 top-2.5 text-red-600/60"
							size={16}
						/>
						<input
							id="lokasi-balai"
							type="text"
							value={props.lokasi}
							onInput={(event) => props.setLokasi(event.currentTarget.value)}
							class=":uno: py-2.5 pr-4 pl-10 text-xs text-[var(--text)] w-full bg-slate-50/50 border border-slate-200 rounded-xl"
							placeholder="Contoh: Aula Balai"
						/>
					</div>
				</div>
				<div class=":uno: flex flex-wrap gap-2 pt-2">
					<button
						type="button"
						onClick={props.onReset}
						class=":uno: flex items-center justify-center font-bold rounded-full text-slate-700 gap-2 px-4 py-3 text-xs bg-[var(--surface-alt)]"
					>
						<FiRotateCcw size={16} /> Reset Formulir
					</button>
					<button
						type="submit"
						class=":uno: flex items-center justify-center font-bold rounded-full text-orange-50 gap-2 px-4 py-3 text-xs bg-primary/80"
					>
						<FiSave size={16} /> Simpan Kegiatan Balai
					</button>
				</div>
			</div>
		</div>
...
export default function FormKegiatanPage() {
	...
	const [formType, setFormType] = createSignal("pegawai");
	const [currentUser, setCurrentUser] = createSignal(null);
	const [tanggalBalai, setTanggalBalai] = createSignal("");
	const [jamBalai, setJamBalai] = createSignal("");
	...
	const handleSubmit = async (e) => {
		...
		if (formType() === "balai") {
			if (!canCreateBalai()) {
				alert(
					"Akses ditolak. Hanya admin yang dapat mengelola kegiatan balai.",
				);
				setFormType("pegawai");
				return;
			}
			const payloadBalai = {
				...
				tanggal: tanggalBalai(),
				jam: jamBalai(),
				lokasi: lokasiInput().trim(),
			};

			if (
				!payloadBalai.nama_kegiatan ||
				!payloadBalai.tanggal ||
				!payloadBalai.jam ||
				!payloadBalai.lokasi
			) {
				alert("Nama kegiatan, tanggal, jam, dan lokasi wajib diisi.");
				return;
			}

			try {
				await createKegiatanBalai(payloadBalai);
				navigate("/display");
			} catch (error) {
				console.error("Submit kegiatan balai gagal", error);
				alert(error.message || "Gagal menyimpan kegiatan balai.");
			}
			return;
		}
		...
		if (
			...
			alert(
				"Nama kegiatan, kategori, waktu mulai, waktu selesai, dan lokasi wajib diisi.",
			);
			return;
		}
	...
	const canCreateBalai = () => currentUser()?.role === "admin";
	...
	const selectedEmployees = () =>
		pegawaiList().filter((p) => selectedPegawaiIds().includes(p.id));
	...
	const handleSelectAllPegawai = () =>
		setSelectedPegawaiIds(filteredPegawai().map((p) => p.id));
	...
	return (
		<>
			<MasterPegawaiModal
				...
				onClose={() => {
					...
					if (currentQuery.get("openEmployeeManager") === "1") {
						...
						navigate(
							`/kegiatan/baru${currentQuery.toString() ? `?${currentQuery.toString()}` : ""}`,
							{
								replace: true,
							},
						);
			...
			<div class=":uno: animate-fade-in">
				...
				<Show when={canCreateBalai()}>
					<div class=":uno: layout-light mt-5 rounded-3xl p-4 bg-[var(--surface)] shadow-md">
						<p class=":uno: text-xs font-bold text-[var(--title)] mb-2">
						...
						<div class=":uno: grid grid-cols-1 sm:grid-cols-2 gap-2">
							...
							>
								Form Kegiatan Pegawai
								<span class=":uno: block text-[10px] font-normal text-slate-500 mt-1">
									Kegiatan dengan penugasan pegawai
								</span>
							</button>
							<button
								type="button"
				...
				<form onSubmit={handleSubmit}>
					<Show when={formType() === "balai"}>
						<BalaiForm
							namaKegiatan={namaKegiatan()}
							setNamaKegiatan={setNamaKegiatan}
							tanggal={tanggalBalai()}
							setTanggal={setTanggalBalai}
							jam={jamBalai()}
							setJam={setJamBalai}
							lokasi={lokasiInput()}
							setLokasi={setLokasiInput}
													...
													{(cat) => (
														...
														>
															...
															<div class=":uno: flex flex-col text-left">
																<span class=":uno: font-bold text-[var(--title)] text-xs">
																	{cat.long}
																</span>
																<span class=":uno: text-[10px]">
																	Kategori {cat.label}
																</span>
															</div>
									...
									<div class=":uno: space-y-4">
										...
										<div>
											...
											<textarea
												...
												onInput={(e) =>
													setHasilKegiatanInput(e.currentTarget.value)
												}
								...
								<div class=":uno: layout-dark mt-5 lg:hidden space-y-4">
									<div class=":uno: bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 text-white rounded-3xl p-6 sm:p-7 shadow-2xl border border-slate-800 space-y-5 relative overflow-hidden">
										...
										<div class=":uno: space-y-3 border-b border-slate-800 pb-4">
												...
												<div class=":uno: flex items-center space-x-3">
													...
													<div>
														...
														<p class=":uno: text-[10px] text-slate-400">
															Pilih pegawai pelaksana tugas kegiatan
														</p>
													</div>
												</div>
											...
											{canManageMaster() && onOpenEmployeeManager && (
										...
										<div class=":uno: space-y-2.5">
											...
											<div class=":uno: relative">
												...
												<input
													...
													value={searchPegawai()}
													onInput={(e) =>
														setSearchPegawai(e.currentTarget.value)
													}
