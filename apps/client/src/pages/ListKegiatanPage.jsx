...
export default function ListKegiatanPage() {
	...
	const [dataKegiatan, { refetch: refetchKegiatan }] =
		createResource(getKegiatanAll);
		...
		let res = all.filter((item) => {
			if (
				...
				String(item.kategori || "").toUpperCase() !==
					String(kategoriFilter()).toUpperCase()
			)
				return false;
			if (
			...
			if (pegawaiFilter() !== "semua") {
				...
				if (!item.pegawai || !item.pegawai.some((p) => Number(p.id) === pid))
					return false;
			}
			...
			if (q) {
				...
				const inPegawai = (item.pegawai || []).some(
	...
	return (
		<>
			<PrintPdfModal
				...
				subtitle="Siapkan daftar kegiatan untuk dicetak atau disimpan dalam format PDF."
				items={filteredKegiatan()}
			/>
				...
				<div class=":uno: space-y-6">
					...
					<div class=":uno: layout-dark grid grid-cols-2 lg:grid-cols-4 gap-3.5">
						<div class=":uno: p-4 rounded-3xl bg-white text-slate-900 border border-slate-200 flex items-center space-x-3.5 transition-all duration-200 ease-out transform-gpu hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-700/10">
							...
							<div>
								<p class=":uno: text-[10px] font-extrabold uppercase text-slate-500 tracking-wider">
									Total Kegiatan
								</p>
								<p class=":uno: text-xl text-slate-900 font-black">
									{dataKegiatan.loading ? "..." : dataKegiatan()?.length}
									<span class=":uno: text-xs font-normal opacity-70 pl-1.5">
										agenda
									</span>
								</p>
							</div>
									...
									<For each={filteredKegiatan()}>
										{(item, index) => {
											...
											return (
												...
												>
													<td class="py-3.5 px-4 text-center font-mono font-extrabold text-slate-400">
														{index() + 1}
													</td>
