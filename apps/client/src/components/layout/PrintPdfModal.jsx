...
export default function PrintPdfModal(props) {
	const [selectedMonth, setSelectedMonth] = createSignal(
		props.monthValue || new Date().toISOString().slice(0, 7),
	);
	...
	const handlePrint = () => {
		...
		const filteredItems = selectedMonth()
			? reportItems.filter((item) => {
					const itemDate = item?.waktu_mulai
						? new Date(item.waktu_mulai)
						: null;
		...
		doc.setFontSize(16);
		doc.text("LAPORAN KEGIATAN PEGAWAI BPMPTP", pageWidth / 2, 36, {
			align: "center",
		});

		doc.setFontSize(10);
		doc.text(
			`Periode Laporan: ${formatMonthLabel(selectedMonth())}`,
			pageWidth / 2,
			56,
			{ align: "center" },
		);
		doc.text(
			`Dicetak otomatis oleh Sistem Pengelola Kegiatan Pegawai pada ${new Date().toLocaleDateString(
				"id-ID",
				{
					day: "2-digit",
					month: "2-digit",
		...
		const totalPegawai = filteredItems.reduce(
			(count, item) =>
				count + (Array.isArray(item.pegawai) ? item.pegawai.length : 0),
			0,
		);
		const totalDLT = filteredItems.filter(
			(item) => String(item.kategori || "").toUpperCase() === "DLT",
		).length;
		...
		const totalCardWidth =
			cards.length * cardWidth + (cards.length - 1) * cardGap;
		...
		autoTable(doc, {
			...
			didDrawPage: () => {
				...
				doc.text(`Halaman ${pageNumber}`, pageWidth - 54, pageHeight - 18, {
					align: "right",
				});
			},
		});
					...
					<div class=":uno: space-y-5 p-6">
						...
						<div class=":uno: grid gap-3 sm:grid-cols-2">
							<div class=":uno: rounded-2xl border border-slate-200 bg-slate-50 p-4">
								...
								<p class=":uno: mt-2 text-sm font-bold text-slate-900">
									A4 Portrait
								</p>
							</div>
						...
						<div class=":uno: rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs text-slate-600">
							...
							akan dicetak ke dalam dokumen PDF. Pastikan semua data kegiatan
							sudah lengkap sebelum mengekspor dokumen.
						</div>
					</div>
